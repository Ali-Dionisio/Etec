...
--
-- Host: 127.0.0.1:3306
-- Tempo de geração: 23-Abr-2023 às 22:02
...
-- Eventos
--
DROP EVENT IF EXISTS `apagarEventoPassado`$$
CREATE DEFINER=`root`@`localhost` EVENT `apagarEventoPassado` ON SCHEDULE EVERY 10 SECOND STARTS '2023-04-19 13:52:26' ON COMPLETION NOT PRESERVE ENABLE DO DELETE FROM tb_salao WHERE data_reserva <= sysdate() and hora_fim <= TIME(sysdate())$$
